...
def put(userrecv, conn):
    print("In put function in server 1.")
    while True:   #try:  
        ...
        if (len(dataDFS1)>0):
            ...
            print("Detailed description of data for Server 1:", filename," ",fnumber1," ",fnumber2," ",indfilesize)
        ...
        if (len(chunk2)>0):
            ...
            break
    print("Server 1 done with PUT function!")
    
def get(userrecv, conn):
    print("In get function in server 1.")
    ...
    time.sleep(2)
    print("Server 1 done with GET function!")
    conn.close()
    
def list(userrecv, conn):
    print("In list function in server 1.")
    path=".\\"+directory1
    ...
    for i in files3:
        ...
        time.sleep(4)
    conn.close()
    print("Server 1 done with LIST function!") 
            
def check(data):
    if data in userpwd:
        print("In check function in server 1.")
        return "True"
    else:
        return "False"
...
def thread(conn, clientaddr):
    try:
        while True:
            print("In thread function in server 1.")  #try:
...
if(len(sys.argv)==3):
    ...
    print("Error: Please provide sys.argv[0]=.py file name, sys.argv[1]=/DFS1 and sys.argv[2]=10001!")
    sys.exit()